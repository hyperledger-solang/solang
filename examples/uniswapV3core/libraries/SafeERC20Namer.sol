...
library SafeERC20Namer {
    function bytes32ToString(bytes32 x) pure private returns (string memory) {
        ...
        bytes memory bytesStringTrimmed = new bytes(uint32(charCount));
    ...
    function callAndParseStringReturn(address token, bytes4 selector) private returns (string memory) {
