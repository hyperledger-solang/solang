...
contract TickMath is ITickMath {
    ...
    function getSqrtRatioAtTick(int24 tick) public pure returns (uint160 sqrtPriceX96) {
        ...
        sqrtPriceX96 = uint160((ratio >> 32) + (ratio % (uint160(1) << 32) == 0 ? 0 : 1));
    }
    ...
    function getTickAtSqrtRatio(uint160 sqrtPriceX96) public pure returns (int24 tick) {
        ...
        f = 0;
        if (r > 0xFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF) f = 1 << 7;
        msb = msb | f;
        r = r >> f;

        f = 0;
        if (r > 0xFFFFFFFFFFFFFFFF) f = 1 << 6;
        msb = msb | f;
        r = r >> f;

        f = 0;
        if (r > 0xFFFFFFFF) f = 1 << 5;
        msb = msb | f;
        r = r >> f;
