...
contract OracleEchidnaTest {
    ...
    function echidna_AlwaysInitialized() external view returns (bool) {
        bool isInitialized = oracle.observations(0).initialized;
    ...
    function echidna_canAlwaysObserve0IfInitialized() external returns (bool) {
        if (!initialized) {
            return true;
        }
        uint32[] memory arr = new uint32[](1);
        arr[0] = 0;
        (bool success, ) = address(oracle).staticcall(abi.encodeWithSignature("observe(uint32[])", arr));
        return success;
    }
    ...
    function checkTwoAdjacentObservationsTickCumulativeModTimeElapsedAlways0(uint16 index) external view {
        ...
        Oracle.Observation observation0 = oracle.observations(index == 0 ? cardinality - 1 : index - 1);
        uint32 blockTimestamp0 = observation0.blockTimestamp;
        int56 tickCumulative0 = observation0.tickCumulative;
        bool initialized0 = observation0.initialized;

        Oracle.Observation observation1 = oracle.observations(index);
        uint32 blockTimestamp1 = observation1.blockTimestamp;
        int56 tickCumulative1 = observation1.tickCumulative;
        bool initialized1 = observation1.initialized;
