...
contract OracleTest {
    ...
    function initialize(InitializeParams calldata params) external {
        ...
        (uint16 _cardinality, uint16 _cardinalityNext) = observations.initialize(params.time);

        cardinality = _cardinality;
        cardinalityNext = _cardinalityNext;
    }
    ...
    function update(UpdateParams calldata params) external {
        ...
        (uint16 _index, uint16 _cardinality) = observations.write(index, time, tick, liquidity, cardinality, cardinalityNext);
        index = _index;
        cardinality = _cardinality;
        tick = params.tick;
        liquidity = params.liquidity;
    ...
    {
        (tickCumulatives, secondsPerLiquidityCumulativeX128s) = observations.observe(time, secondsAgos, tick, index, liquidity, cardinality);
    }

    function getGasCostOfObserve(uint32[] calldata secondsAgos) external view returns (uint256) {
        uint256 gasBefore = gasleft();
        (,) = observations.observe(time, secondsAgos, tick, index, liquidity, cardinality);
