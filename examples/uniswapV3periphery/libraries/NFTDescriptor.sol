...
import '../uniswapV3core/libraries/ITickMath.sol';
import '../uniswapV3core/libraries/BitMath.sol';
import '../uniswapV3core/libraries/IFullMath.sol';
import '../openzeppelin/utils/Strings.sol';
import '../openzeppelin/utils/math/SafeMath.sol';
import '../openzeppelin/utils/math/SignedSafeMath.sol';
import '../uniswapV3core/libraries/Base64.sol';
...
library NFTDescriptor {
    ...
    address internal constant TickMath = address(0x324fa16815ed8e32ee7eca91353a8a285f90aec9);
    ...
    function constructTokenURI(ConstructTokenURIParams memory params) public pure returns (string memory) {
        ...
        string memory image = Base64.encode(string(generateSVGImage(params)));
    ...
    function escapeQuotes(string memory symbol) internal pure returns (string memory) {
        ...
        for (uint8 i = 0; i < symbolBytes.length; i++) {
            byte _symbolBytes = symbolBytes[i];
            if (symbol == byte('"')) {
                quotesCount++;
            }
        }
    ...
    {
        int24 _tickLower = params.tickUpper;
        if (!params.flipRatio) {
            _tickLower = params.tickLower;
        }

        int24 _tickUpper = params.tickLower;
        if (!params.flipRatio) {
    ...
    function tickToDecimalString(
        ...
        if (tick == (MIN_TICK / tickSpacing) * tickSpacing) {
            return !flipRatio ? string('MIN') : string('MAX');
        } else if (tick == (MAX_TICK / tickSpacing) * tickSpacing) {
            return !flipRatio ? string('MAX') : string('MIN');
        } else {
            uint160 sqrtRatioX96 = ITickMath(TickMath).getSqrtRatioAtTick(tick);
            if (flipRatio) {
                sqrtRatioX96 = uint160(uint256(uint256(1) << 192).div(sqrtRatioX96));
            }
    ...
    function adjustForDecimalPrecision(
        ...
        if (difference > 0 && difference <= 18) {
            if (baseTokenDecimals > quoteTokenDecimals) {
                ...
                if (difference % 2 == 1) {
                    adjustedSqrtRatioX96 = IFullMath(FullMath).mulDiv(adjustedSqrtRatioX96, sqrt10X128, 1 << 128);
                }
            } else {
                adjustedSqrtRatioX96 = sqrtRatioX96.div(10**(difference.div(2)));
                if (difference % 2 == 1) {
                    adjustedSqrtRatioX96 = IFullMath(FullMath).mulDiv(adjustedSqrtRatioX96, 1 << 128, sqrt10X128);
                }
            }
        } else {
    ...
    function fixedPointToDecimalString(
        ...
        uint256 value = IFullMath(FullMath).mulDiv(adjustedSqrtRatioX96, adjustedSqrtRatioX96, 1 << 64);
        ...
        if (priceBelow1) {
            ...
            value = IFullMath(FullMath).mulDiv(value, 10**44, 1 << 128);
        } else {
            // leave precision for 4 decimal places + 1 place for rounding
            value = IFullMath(FullMath).mulDiv(value, 10**5, 1 << 128);
        }
