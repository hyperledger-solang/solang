...
impl Config {
    ...
    pub fn optimizer(&self) -> Optimizer {
        ...
        let details = if self.optimizer {
            self.optimizer_details.clone()
        } else {
            None
        };
...
mod tests {
    ...
    fn test_resolve_etherscan_with_chain() {
        figment::Jail::expect_with(|jail| {
            ...
            let config = Config::load();
            assert!(config
                .get_etherscan_config_with_chain(None::<u64>)
                .unwrap()
                .is_none());
            assert!(config
    ...
    fn test_extract_etherscan_config_by_chain_and_alias() {
        figment::Jail::expect_with(|jail| {
            ...
            let mumbai = config
                .get_etherscan_config_with_chain(Option::<u64>::None)
                .unwrap()
                .unwrap();
            assert_eq!(mumbai.key, "https://etherscan-mumbai.com/".to_string());
    ...
    fn test_solc_req() {
        figment::Jail::expect_with(|jail| {
            ...
            assert_eq!(
                config.solc,
                Some(SolcReq::Version("0.8.12".parse().unwrap()))
            );
            ...
            assert_eq!(
                config.solc,
                Some(SolcReq::Version("0.6.6".parse().unwrap()))
            );
            Ok(())
        });
    }
