...
use std::{collections::HashSet, ffi::OsString, fs, path::PathBuf, process::exit, str::FromStr};
use toml::Table;
...
impl Configurations {
    ...
    fn parse_target(cfg: &Table) -> Result<Target, String> {
        ...
        match target_name {
            ...
            _ => {
                return Err("provided invalid target".to_owned());
            }
        }
    }
...
"#;
    ...
    assert_eq!(config.0, true);
    assert_eq!(config.1, false);
