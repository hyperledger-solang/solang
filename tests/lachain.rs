    ...
    fn invoke_index(
        ...
        match FromPrimitive::from_usize(index) {
            ...
            Some(Extern::callDataCopy) => {
                let input_offset = args.nth_checked::<u32>(0)? as usize;
                let input_len = args.nth_checked::<u32>(1)? as usize;
                let dest = args.nth_checked::<u32>(2)?;
            ...
            Some(Extern::setReturn) => {
                ...
                println!("setReturn: {} {}", len, hex::encode(&output));

                self.vm.output = output;

                Ok(None)
            }
            Some(Extern::systemHalt) => {
            ...
            Some(Extern::create) => {
                ...
                match module.invoke_export("start", &[], self) {
                    Err(wasmi::Error::Trap(trap)) => match trap.kind() {
                        ...
                        _ => panic!("fail to invoke start via create: {}", trap),
                    },
                    Ok(_) => {}
                    Err(e) => panic!("fail to invoke start via create: {}", e),
                }
...
impl ModuleImportResolver for TestRuntime {
    fn resolve_func(&self, field_name: &str, signature: &Signature) -> Result<FuncRef, Error> {
        let index = match field_name {
            "get_call_size" => Extern::getCallDataSize,
            "copy_call_value" => Extern::callDataCopy,
            "set_return" => Extern::setReturn,
            "system_halt" => Extern::systemHalt,
            "save_storage" => Extern::storageStore,
            "load_storage" => Extern::storageLoad,
            "create" => Extern::create,
            "invoke_contract" => Extern::call,
            "copy_return_value" => Extern::returnDataCopy,
            "get_return_size" => Extern::getReturnDataSize,
            "get_msgvalue" => Extern::getCallValue,
            "get_address" => Extern::getAddress,
            "get_external_balance" => Extern::getExternalBalance,
            "write_log" => Extern::log,
            "crypto_keccak256" => Extern::keccak256,
            "crypto_ripemd160" => Extern::ripemd160,
            "crypto_sha256" => Extern::sha256,
            _ => {
...
impl TestRuntime {
    fn create_module(&self, code: &[u8]) -> ModuleRef {
        ...
        ModuleInstance::new(
            &module,
            &ImportsBuilder::new().with_resolver("env", self),
        )
            ...
            Err(wasmi::Error::Trap(trap)) => match trap.kind() {
                TrapKind::Host(host_error) => {
                    assert!(
                        ...
                        "fail to invoke start: {}",
                        host_error
                    );
                }
                _ => panic!("fail to invoke start: {}", trap),
            },
            Ok(Some(RuntimeValue::I32(0))) => {}
            Ok(Some(RuntimeValue::I32(ret))) => panic!("start returns: {}", ret),
            Err(e) => panic!("fail to invoke start: {}", e),
            Ok(None) => panic!("fail to invoke start"),
            _ => panic!("fail to invoke start, unknown"),
        }
    ...
    fn do_constructor(&mut self, args: &[Token]) -> bool {
        ...
        self.accounts
            .insert(self.vm.cur, (self.contracts.last().unwrap().to_vec(), 0));
