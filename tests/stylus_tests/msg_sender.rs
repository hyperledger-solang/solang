...
fn msg_sender() {
    ...
    let emit_re = Regex::new(r"\<emit\>").unwrap();
    let library_re = Regex::new(r"\<library\>").unwrap();
    ...
    for result in WalkDir::new("testdata/solidity/test/libsolidity/semanticTests") {
        ...
        if !msg_sender_re.is_match(&contents) {
            continue;
        }
        if assembly_re.is_match(&contents)
            || emit_re.is_match(&contents)
            || library_re.is_match(&contents)
        {
            continue;
        }
